...
double temp=-123.456, humi=-0, dist=100.123;
unsigned int illu=65535;

unsigned int wendu[]={2, 3, 0xFFFF};
unsigned int du[]={1, 0xFFFF};
unsigned int shidu[]={4, 3, 0xFFFF};
unsigned int guangzhaodu[]={5, 6, 3, 0xFFFF};
...
void showData()
{
    ...
    lcd.putBigString(4,0,guangzhaodu);
    lcd.putBigString(6,0,juli);
    
    
    toString(uiabs(temp), temp<0, buff, 4, ' ');
    lcd.putAsciiString(0,48,buff);
    lcd.putAsciiString(0,80,".");
    toString(uiabs(temp*100), temp<0, buff, 2, '0');
    lcd.putAsciiString(0,88,buff);
    lcd.putBigString(0,104,du);
    lcd.putAsciiString(0,112,"C");
    
    toString(uiabs(humi), humi<0, buff, 3, ' ');
    lcd.putAsciiString(2,56,buff);
    lcd.putAsciiString(2,80,".");
    toString(uiabs(humi*100), humi<0, buff, 2, '0');
    lcd.putAsciiString(2,88,buff);
    lcd.putAsciiString(2,104,"%");
    ...
    lcd.putAsciiString(6,56,buff);
    lcd.putAsciiString(6,80,".");
    toString(uiabs(dist*100), dist<0, buff, 2, '0');
    lcd.putAsciiString(6,88,buff);
    lcd.putAsciiString(6,104,"cm");
}
